...
//Added by Nadine 18.07
    ...
    public void prescInsert(String prescription, Integer patientId, String drugId) {
        this.openConnection();
        try {
            DB.db.openConnection();
            StringBuffer st = new StringBuffer();
...
//added by Nadine 25 July
    ...
    public ResultSet prescriptionByPatientId(String id) {
        ...
        //added by VAlerii 31 July
     public ResultSet testIdFromSchedule(int ptid) {
        ResultSet res = null;
        try {
            PreparedStatement ins =
                    c.prepareStatement("SELECT testid FROM schedule WHERE patientid = ?");
            ins.setInt(1, ptid);
            res = ins.executeQuery();
        } catch (Exception ex) {
            ex.printStackTrace();
